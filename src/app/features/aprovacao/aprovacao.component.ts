...
@Component({
  ...
  styleUrl: './aprovacao.component.css',
})
export class AprovacaoComponent implements OnInit, AfterViewInit {
  ...
  async ngAfterViewInit(): Promise<void> {
    ...
    const aprovador = this.variaveisProcesso['proximoAprovador'] as string;
    ...
    const gruposFiltradosSapiens = gruposSapiens.filter((grupo: GrupoModel) => grupo.nomapr === aprovador);
    this.possuiGruposSapiens = !!gruposFiltradosSapiens.length;
    if (this.possuiGruposSapiens) this.gruposSapiensComponent.preencherGrupos(gruposFiltradosSapiens);
    ...
    const gruposFiltradosHcm = gruposHcm.filter((grupo: GrupoModel) => grupo.nomapr === aprovador);
    this.possuiGruposHcm = !!gruposFiltradosHcm.length;
    if (this.possuiGruposHcm) this.gruposHcmComponent.preencherGrupos(gruposFiltradosHcm);
  ...
  async enviaEtapa(processStep: WfProcessStep): Promise<WfFormData> {
    ...
    if (['Solicitar Revisão', 'Reprovar'].includes(acaoEscolhida)) {
      ...
      usuario = `${this.workflowService.getUser().fullname} (${this.workflowService.getUser().username}) - APROVADOR GRUPO`;
    } else {
      this.dadosAprovacao.setNecessitaJustificativa(false);

      const aprovadoresString = this.variaveisProcesso['aprovadores'] as string;
      aprovadores = aprovadoresString.split(';');
      aprovadores.shift();
      if (aprovadores.length) {
        proximoAprovador = aprovadores[0];
      }
    }
    ...
    const dadosEtapa = {
      ...
      proximoAprovador: proximoAprovador,
